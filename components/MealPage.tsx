      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <div className="relative aspect-video rounded-lg overflow-hidden mb-4">
            ...
            />
            <h1 className="absolute bottom-12 left-4 text-white text-3xl font-bold mb-2 bg-slate-800/20 shadow-red-900">
              {meal.strMeal}
            </h1>
          ...
          <div className="mb-2 p-4 rounded-lg shadow-inner">
            ...
            <Link href={meal.strSource} target="_blank" rel="noopener noreferrer">
            ...
            <div className='flex flex-col h-full w-full'>
              <ol className="list-none list-inside gap-y-2 gap-x-4 grid grid-cols-2 ">
                {ingredients.map(({ ingredient, measure, image }, index) => (
                  <li key={index} className='flex flex-row gap-2 justify-start items-center'>
                    <Image src={image} alt={ingredient} width={50} height={50} className='w-12 h-12'/>
                    <div className="flex-grow">
                  {ingredient &&<p className="font-medium">{ingredient}</p>}
                  {measure &&<p className="text-sm text-muted-foreground">{measure}</p>}
                </div>
                  </li>
                ))}
              </ol>
